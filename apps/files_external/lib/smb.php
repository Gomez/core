	...
	public function fopen($path, $mode) {
		...
		try {
			switch ($mode) {
				case 'r':
				case 'rb':
					return $this->share->read($path);
				case 'w':
				case 'wb':
					return $this->share->write($path);
				case 'a':
				case 'ab':
				case 'r+':
				case 'w+':
				...
				case 'c+':
					...
					$share = $this->share;
					return CallBackWrapper::wrap($source, null, null, function () use ($tmpFile, $path, $share) {
						$share->put($tmpFile, $path);
						unlink($tmpFile);
					});
			}
			return false;
